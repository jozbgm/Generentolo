...
- **Shots Storyboard** — Seedance 2.0 video prompt engine: load reference images, write a narrative brief, and generate a full shot-by-shot Seedance prompt (timeline + effects inventory + density map + energy arc) plus a Nano Banana 2 frame prompt per shot; selective references per shot, physical/product fidelity memory across all frames, graphic-shot detection (no camera params on logos/motion graphics); edit individual frame prompts inline — edits are preserved in the generation queue; regenerate with new references at any time
